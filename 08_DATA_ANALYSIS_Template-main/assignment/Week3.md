...
1. **데이터 정제**는 수집된 데이터에서 잘못된 부분을 고치거나 제거하여 필요한 데이터를 준비하는 과정을 말한다. 데이터가 올바르게 정제되지 못하면 분석된 결과를 왜곡시킬 수 있으며 잘못된 의사 결정을 초래할 수 있다.
2. 데이터를 정제하는 과정과 데이터 분석 및 머신러닝에 적합한 형태로 데이터를 변환하는 과정을 합쳐서 **데이터 랭글링 및 데이터 먼징**이라고 한다.
3. 판다스의 데이터프레임과 인덱스를 하나의 값과 비교하면 데이터프레임과 인덱스에 있는 모든 원소와 비교하는데, 이를 **원소별 비교**라고 한다. 비교한 결과는 True 혹은 False로 구성된 **불리언 배열**로 변환된다.
